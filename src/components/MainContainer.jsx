...
function MainContainer() {
  return (
    <section className="grid grid-cols-1 lg:grid-cols-2 gap-2 md:px-16">
      ...
      <div className="py-2 flex-1 flex items-center relative w-full">
        <img
          src={HeroBg}
          className=" ml-auto h-500 w-full md:w-3/4 lg:h-650"
          alt="hero-bg"
        />

        <div className="w-full h-full absolute top-0 left-0 flex items-center justify-center lg:px-16  pt-8  lg:gap-16 md:gap-2 gap-8 flex-wrap">
          {heroData &&
            heroData.map((n) => (
              <div
                key={n.id}
